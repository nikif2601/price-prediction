...
def fetch_weather_history(lat: float, lon: float, start: datetime, end: datetime) -> pd.DataFrame:
    """Изтегля исторически дневни метео данни (темп., вятър, радиация)."""
    records = []
    date = start
    while date <= end:
...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for lag in [1,7,365]:
        for h in range(24): data[f'hour_{h}_lag{lag}'] = data[f'hour_{h}'].shift(lag)
...
def prepare_dataset(df_feat: pd.DataFrame, horizon_days=7) -> tuple:
    """Подготвя X,y за обучение на мулти-часова прогноза за следващата седмица."""
    # Предсказваме 24*horizon дни
    y_cols = []
    for d in range(1, horizon_days+1):
        for h in range(24): y_cols.append(f'hour_{h}_lead{d}')
    # Добавяме lead колони
    df = df_feat.copy()
    for d in range(1, horizon_days+1):
...
def train_multioutput_model(X: pd.DataFrame, y: pd.DataFrame):
    ...
    model.fit(Xs,y)
    joblib.dump(model,'multi_day_model.pkl'); joblib.dump(scaler,'multi_day_scaler.pkl')
    print(f"CV MAE: {np.mean(maes):.3f}")
    return model, scaler

# ==================== ПРОГНОЗА ЗА СЕДМИЦА============
def forecast_week(df_hist: pd.DataFrame, model, scaler):
    # Обединяване на история + прогноза за време
...
def run_analysis(df: pd.DataFrame):
    ...
    # Сезонна декомпозиция на среднодневна цена
